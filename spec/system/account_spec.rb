...
describe "Account" do
  ...
  let(:phone_number) { { "enabled" => true, "pattern" => phone_number_pattern, "placeholder" => nil } }
  ...
  context "when on the account page" do
    ...
    # Temporairement désactivé - problème de certificat SSL avec validateur W3C
      ...
      context "with phone number pattern blank" do
        ...
        it "updates the user's data" do
          within "form.edit_user" do
            ...
            fill_in :user_name, with: "Nikola Tesla"
            fill_in :user_personal_url, with: "https://example.org"
            fill_in :user_about, with: "A Serbian-American inventor."
    ...
    # Tests for disabled extra fields
  ...
  context "when on the notifications page in a PWA browser" do
    ...
    context "when VAPID keys are set" do
      before do
        allow(Decidim).to receive_messages(vapid_public_key: vapid_keys[:public_key], vapid_private_key: vapid_keys[:private_key])
